...
struct Shop createAndStockShop()
{
    ...
    while ((read = getline(&line, &len, fp)) != -1) {		
		...
		struct CustomerWants stockItem = { product, quantity };
...
int main(void) 
{
	...
	struct CustomerWants StockA = { productA, 12 };
	struct CustomerWants StockB = { productB, 5 };
	struct CustomerWants StockC = { productC, 2 };
	
	struct CustomerWants array[] = { StockA, StockB, StockC }; 
